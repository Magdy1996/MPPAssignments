...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		list.stream().sorted(Comparator.comparing((Integer x)-> Math.abs(x)).thenComparing(x-> x))
	...
	public static void ordering2(List<String> words) {
		words.stream().sorted(Comparator.comparing((String s) -> new StringBuilder(s).reverse())).forEach(System.out::println);
